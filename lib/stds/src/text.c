...
static TTF_Font *get_font( char *, uint16_t );
static void      load_fonts();
static void      add_font( char *, uint16_t );
...
void
draw_text( float x, float y, uint8_t r, uint8_t g, uint8_t b, char *font_string, uint16_t font_size,
           const char *text, ... ) {
